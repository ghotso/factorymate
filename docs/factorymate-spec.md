...
**FRM safe reconnect (poller gate):** When a fast poll is unreachable (connection refused, timeout, or any endpoint error), background FRM HTTP polling stops entirely until recovery completes. The poller enters `recovery_phase = down` and TCP-probes the game server using **Settings → Connection → Internal game server connection** `game_api_host`/`game_api_port` (local endpoint reachable from the FactoryMate container — not the player-facing join host). Probe interval is 5s (not user-configurable). When TCP connect succeeds, `recovery_phase = recovering` and the poller waits `app_settings.frm_recovery_grace_seconds` (default 60, editable in Settings → General) before resuming FRM — a successful TCP probe does **not** mean FRM is ready. After grace, a single `getSessionInfo` probe runs; on success the full fast poll resumes and `recovery_phase` returns to `healthy`. On `getSessionInfo` or full-poll failure, return to `down` without a second `server_offline` (already offline). Slow poll skips all FRM calls while `recovery_phase != healthy`. Admin-initiated FRM test (`POST /api/settings/frm/test`) and settings PUT validation are not gated. `GET /api/status` exposes `recoveryPhase` (`healthy` | `down` | `recovering`) alongside `serverOnline`. On FactoryMate startup, if `server_online = false`, the gate initializes in `down` to avoid an immediate FRM hit during an ongoing outage. API token for the internal connection is generated in the dedicated server console with `server.GenerateAPIToken` (see `docs/guide/frm-connectivity.md`).
...
| `/resource-sink` | viewer, admin | Current A.W.E.S.O.M.E. Sink status (coupon count, progress-to-next-coupon, total points) as cards, plus two separate interval-selectable historical charts — coupon inventory (`numCoupon`) and lifetime total points (`totalPoints`) — each with its own Y-axis scale, backed by `resource_sink_snapshots` — deliberately not a passthrough of FRM's own fixed rolling graph (see §4.1) | Cards, two Recharts charts with shared date-range control |
...
| `/power` | — | `Table` (Data Table pattern, full battery/power column set; row click selects circuit for chart), `Progress` (capacity/consumption bars), `Badge`/`Alert` (tripped state), `Chart` (Recharts wrapper, per-circuit historical trend), `Calendar`+`Popover`/`Select` (date-range via `IntervalPicker`, same pattern as `/production`) | Inactive circuit groups (all displayed metrics zero) are hidden in the UI; stale `circuit_state` rows are pruned on poll. Row-click chart selection mirrors `/production`. |
| `/resource-sink` | — | `Card` (current coupon count, progress, totals), `Chart` (Recharts wrapper, two separate historical trends — coupons and total points — with independent Y-axis scales), `Calendar`+`Popover`/`Select` (shared date-range, same pattern as `/production`) | None |
